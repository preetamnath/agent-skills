...
4. **Type-check, do NOT MCP-validate.** Run `tsc --noEmit` (or the project's equivalent) against the artifact. The `polaris-checkout-extensions` MCP validator is scoped to modern web-component extensions and rejects every post-purchase component as "not a Polaris web component" — **never call `validate_component_codeblocks` for post-purchase code**.
