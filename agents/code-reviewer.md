...
Return a `ReviewOutput` envelope conforming to the [Output Schema](#output-schema) below. Set every finding's `validated_by` to `reviewer`; leave `verdict` and `evidence` null.
