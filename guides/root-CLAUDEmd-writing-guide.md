...
- **Read-when-relevant routes:** `task or question → authoritative file`.
...
| Task-specific product, design, operations, or decision context | Maintained document named by a root or workflow instruction |
...
A cross-module topic does not justify a separate document. Create or keep one only when a named task needs it before work, a root or workflow instruction names it, and a workflow keeps it current; otherwise decompose its facts into local comments, ancestor instructions, or exact path rules.
...
1. Read the current root file and every source that may own its facts: user instructions, nested `CLAUDE.md` files, path-scoped rules, skills, workflows, and maintained task documents.
...
4. Build the recommended shape from the admission test and ownership table; organize routes by task trigger, not document inventory.
...
- Can an agent reach every maintained source—task context, active work, operations, and close-out—from a matching read-when-relevant route?
