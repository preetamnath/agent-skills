...
- **[durable-docs-update](skills/durable-docs-update/)** — After a coding task or plan, sweep the changed files' code comments — correcting or deleting stale ones — then turn the facts worth keeping into durable-doc adds/updates/trims (CLAUDE.md, ARCHITECTURE.md, .claude/rules); auto-applies high-confidence ones, asks approval for the rest.
- **[execute-chat](skills/execute-chat/)** — Execute multi-task work agreed in the chat — no spec or plan.md — through a readiness gate, dependency-ordered waves of parallel Opus subagents (the parent verifies each wave's real diff), a two-pass review at a recommended cadence, a working gate, then one durable-docs pass that sweeps the comments and syncs the docs; every stage lands in a task ledger. The light sibling of execute-plan.
...
- **[explain-simply](skills/explain-simply/)** — Re-explain your last response in the simplest language: one idea per sentence, plain words, and an ASCII diagram for anything with shape — then keep that style for every response after it.
