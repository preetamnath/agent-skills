...
Print the planned list and confirm via `AskUserQuestion` ("Generate these" / "Adjust") before spending generation effort:

```
**Planned mockups ([N] — PREVIEW | COMPARE):**
- [screen/state | direction — its bet]
```
...
You assemble the final artifact yourself. When Step 3 fanned out:
1. **Merge** the returned files into it — isolate each screen (e.g. `<iframe srcdoc>` or scoped wrappers) so styles don't collide.
2. **Re-run the QC floor** on the merged page.
3. **Discard the subagent files** — they're inputs, not deliverables.
...
- **COMPARE** → one `options.html`: directions side-by-side, each next to the real sibling control (and a current/failed state where one exists); each card carries its fields; the agent's **verdict on top**.
...
- **PREVIEW** → the user reviews; while the review is live, iterate on the same artifact in place; once the design ships, the snapshot rule applies.
...
COMPARE cards add:
- **Best for** — where it best serves the drafted job-story, often its *without* clause (the struggling moment the job-story names).
